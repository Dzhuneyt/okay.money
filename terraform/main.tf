provider "aws" {
  region = var.aws_region
}
data "aws_region" "current" {

}
//module "ecs_cluster" {
//  source = "github.com/Dzhuneyt/Terraform-Module-AWS-ECS-Cluster?ref=v1.0.1"
//  create_vpc = true
//  aws_region = var.aws_region
//  cluster_name = var.cluster_name
//  subnet_ids = module.vpc.public_subnets
//  # Not needed to access EC2 instances for now
//  ssh_key_name = ""
//  vpc_id = module.vpc.vpc_id
//  instance_type_spot = "t3a.medium"
//  min_spot_instances = "3"
//  max_spot_instances = "5"
//  min_ondemand_instances = "0"
//  max_ondmand_instances = "0"
//  spot_bid_price = "0.0122"
//}

# Allow traffic from the ALB to apps within the cluster
resource "aws_security_group_rule" "allow_alb_traffic_to_apps" {
  from_port                = 0
  protocol                 = "-1"
  security_group_id        = var.cluster_security_group_for_ec2_instances
  source_security_group_id = module.personal_finance.security_group_alb_id
  to_port                  = 0
  type                     = "ingress"
}

module "personal_finance" {
  source          = "./modules/personal_finance"
  cluster_id      = var.cluster_id
  MYSQL_DB        = var.MYSQL_DB
  MYSQL_HOST      = var.MYSQL_HOST
  MYSQL_PASSWORD  = var.MYSQL_PASSWORD
  MYSQL_USER      = var.MYSQL_USER
  domain_name     = var.domain_name
  private_subnets = var.private_subnets
  public_subnets  = var.public_subnets
  version_tag     = var.version_tag
  vpc_id          = var.vpc_id
}

module "rds" {
  source = "./modules/rds"
  allowed_ingress_ips = [
    "${var.vpc_nat_gateway_public_ip}/32"
  ]
}
